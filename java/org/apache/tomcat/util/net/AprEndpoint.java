...
public class AprEndpoint {
	...
	public void init() throws Exception {
		...
		long inetAddress = Address.info(addressStr, family, port, 0, rootPool);
		...
		if (!reverseConnection) {
			...
			// on other platforms this call is noop and will return
			// APR_ENOTIMPL.
			if (deferAccept
					&& (Socket.optSet(serverSock, Socket.APR_TCP_DEFER_ACCEPT, 1) == Status.APR_ENOTIMPL)) {
				deferAccept = false;
			}
		} else {
		...
		if (SSLEnabled) {
			...
			// Create SSL Context
			sslContext = SSLContext.make(rootPool, value, (reverseConnection) ? SSL.SSL_MODE_CLIENT
					: SSL.SSL_MODE_SERVER);
			// SSL renegociation
		...
		synchronized (workers) {
			...
			if ((maxThreads > 0) && (curThreads < maxThreads)) {
				...
				if (curThreadsBusy == maxThreads) {
					log.info(sm.getString("endpoint.info.maxThreads", Integer.toString(maxThreads),
	...
	public class Poller implements Runnable {
		...
		protected void init() {
			...
			// used
			// again as the normal poller could be faster using maintain. It
			// might not
		...
		public void add(long socket, int timeout, boolean read, boolean write, boolean resume,
			...
			synchronized (this) {
				// Add socket to the list. Newly added sockets will wait
				// at most for pollTime before being polled
				if (addList.add(socket, timeout, (read ? SocketInfo.READ : 0)
						| (write ? SocketInfo.WRITE : 0) | (resume ? SocketInfo.RESUME : 0)
						...
						while (info != null) {
							if (info.wakeup()) {
								...
								if (timeouts.remove(info.socket)) {
									...
									if (info.resume()) {
										...
										if (!addToPoller(info.socket, events)) {
											// Can't do anything: close the
											// socket right away
											if (!event
													|| (event && !processSocket(info.socket,
															SocketStatus.ERROR))) {
												Socket.destroy(info.socket);
											}
										} else {
								...
								if (event) {
									...
									// Should never happen, if not event, the
									// socket is always put in
					...
					for (int i = 0; i < pollers.length; i++) {
						...
						if (rv > 0) {
							...
							for (int n = 0; n < rv; n++) {
								...
								if (event) {
									...
									if (((desc[n * 2] & Poll.APR_POLLHUP) == Poll.APR_POLLHUP)
										...
										// Unknown event
										log.warn(sm.getString("endpoint.poll.flags", ""
												+ desc[n * 2]));
										if (!processSocket(desc[n * 2 + 1], SocketStatus.ERROR)) {
											// Close socket and clear pool
											Socket.destroy(desc[n * 2 + 1]);
							...
							if ((errn != Status.TIMEUP) && (errn != Status.EINTR)) {
								...
								log.error(sm.getString("endpoint.poll.fail", "" + errn,
										Error.strerror(errn)));
				...
				if (!deferAccept && options) {
					...
					if ((status != null)
						...
						if (serverSockPool != 0) {
							...
							&& ((options && !setSocketOptions(socket)) || handler.process(socket) == Handler.SocketState.CLOSED)) {
						// Close socket and pool only if it wasn't closed
						// already by the parent pool
						if (serverSockPool != 0) {
							Socket.destroy(socket);
		...
		public boolean add(SendfileData data) {
			...
			try {
				...
				data.fd = File.open(data.fileName, File.APR_FOPEN_READ
						| File.APR_FOPEN_SENDFILE_ENABLED | File.APR_FOPEN_BINARY, 0, data.fdpool);
				data.pos = data.start;
				// Set the socket to nonblocking mode
				Socket.timeoutSet(data.socket, 0);
				while (true) {
					long nw = Socket.sendfilen(data.socket, data.fd, data.pos, data.end - data.pos,
							0);
					if (nw < 0) {
						if (!(-nw == Status.EAGAIN)) {
				...
				try {
					...
					if (rv > 0) {
						for (int n = 0; n < rv; n++) {
							...
							long nw = Socket.sendfilen(state.socket, state.fd, state.pos, state.end
									- state.pos, 0);
							if (nw < 0) {
								// Close socket and clear pool
								remove(state);
	...
	public class WorkerStack {
		 ...
		 * 
		 * @param object
		 *            the object to be appended to the queue (first element).
		 */
