 ...
 * @version $Revision: 1781 $ $Date: 2011-07-13 18:31:38 +0200 (Wed, 13 Jul
 *          2011) $
 */

public class Request implements HttpServletRequest {

	protected static final boolean SESSION_ID_CHECK = Boolean.valueOf(
			System.getProperty("org.apache.catalina.connector.Request.SESSION_ID_CHECK", "true"))
			.booleanValue();

	protected static final boolean WRAPPED_RESPONSE_IN_LOGIN = Globals.STRICT_SERVLET_COMPLIANCE
			|| Boolean.valueOf(
					System.getProperty(
							"org.apache.catalina.connector.Request.WRAPPED_RESPONSE_IN_LOGIN",
							"false")).booleanValue();

	protected static final boolean CHECK_ASYNC = Globals.STRICT_SERVLET_COMPLIANCE
			|| Boolean
					.valueOf(
							System.getProperty("org.apache.catalina.connector.Request.CHECK_ASYNC",
									"true")).booleanValue();

	protected static final boolean USE_PRINCIPAL_FROM_SESSION = Boolean.valueOf(
			System.getProperty("org.apache.catalina.connector.Request.USE_PRINCIPAL_FROM_SESSION",
					"false")).booleanValue();

	protected static final boolean LOCAL_RANDOM = Boolean.valueOf(
			System.getProperty("org.apache.catalina.connector.Request.LOCAL_RANDOM",
					(org.apache.tomcat.util.Constants.LOW_MEMORY) ? "false" : "true"))
			.booleanValue();

	protected static final boolean SEED_WITH_NEXT_BYTES = Boolean
			.valueOf(
					System.getProperty(
							"org.apache.catalina.connector.Request.SEED_WITH_NEXT_BYTES", "true"))
			.booleanValue();
	 ...
	 */
	private HashMap<Object, Object> readOnlyAttributes = new HashMap<Object, Object>();

	/**
	 * The preferred Locales assocaited with this Request.
	 */
	protected ArrayList<Object> locales = new ArrayList<Object>();

	/**
	 * Internal notes associated with this request by Catalina components and
	 * event listeners.
	 */
	private transient HashMap<Object, Object> notes = new HashMap<Object, Object>();

	/**
	 * Servlet 3.0 asynchronous mode context (also used as a flag - when not
	 * null, we're in asynchronous mode).
	 */
	 ...
	 * Async listeners.
	 */
	protected LinkedHashMap<AsyncListener, AsyncListenerRegistration> asyncListeners = new LinkedHashMap<AsyncListener, AsyncListenerRegistration>();
	...
	public Object getAttribute(String name) {
		...
		if (name.equals(Globals.DISPATCHER_TYPE_ATTR)) {
			...
			return (requestDispatcherPath == null) ? getRequestPathMB().toString()
		...
		if (!sslAttributes && isSSLAttribute(name)) {
			sslAttributes = true;
			coyoteRequest.action(ActionCode.ACTION_REQ_SSL_ATTRIBUTE, coyoteRequest);
	...
	public X509Certificate[] getCertificateChain() {
		...
		if ((certs == null) || (certs.length < 1)) {
			coyoteRequest.action(ActionCode.ACTION_REQ_SSL_CERTIFICATE, coyoteRequest);
	...
	public void removeAttribute(String name) {
		...
		for (int i = 0; i < listeners.length; i++) {
			...
			try {
				...
				// Error valve will pick this execption up and display it to
				// user
	...
	public boolean isUserInRole(String role) {
		...
		if (wrapper != null) {
			...
			if ((realRole != null) && realm.hasRole(principal, realRole))
				return (true);
		}
	...
	protected Session doGetSession(boolean create) {
		...
		if (sessionId != null) {
			if (SESSION_ID_CHECK) {
				...
				try {
					...
					// Ignore: one manager is broken, and it will show up
					// elsewhere again
				}
				if (!found) {
					sessionId = null;
		...
		// If there was no cookie with the current session id, add a cookie to
		// the response
		if ((session != null)
				&& context.getCookies()
				&& !(isRequestedSessionIdFromCookie() && (session.getIdInternal()
			...
			if (parts == null && "multipart/form-data".equals(contentType)) {
				try {
					...
					if (context.getLogger().isDebugEnabled()) {
						context.getLogger().debug(sm.getString("coyoteRequest.parseMultipart"), e);
					}
				}
			}
			return;
		...
		try {
			for (FileItem fileItem : upload.parseRequest(getRequest())) {
				if (fileItem.getName() == null) {
					coyoteRequest.getParameters().addParameterValues(fileItem.getFieldName(),
	...
	public AsyncContext startAsync(ServletRequest servletRequest, ServletResponse servletResponse)
		...
		for (AsyncListenerRegistration registration : localAsyncListeners.values()) {
			...
			AsyncEvent asyncEvent = new AsyncEvent(asyncContext, registration.getRequest(),
		...
		for (int i = 0; i < filterChainCount; i++) {
			...
			for (int j = 0; j < n; j++) {
				...
				buf.append("[F] ").append(filterDef.getFilterName()).append(" [")
						...
						.append(filterDef.getAsyncSupported() ? "[A]" : "").append("\r\n");
			}
			if (pos == n) {
				buf.append("-> ");
			} else {
			...
			buf.append("[S] ").append(wrapper.getName()).append(" [")
					...
					.append(wrapper.getAsyncSupported() ? "[A]" : "").append("\r\n");
		}
		return buf.toString();
	}
	...
	public class AsyncContextImpl implements AsyncContext {
			...
			if (servletRequest == getRequestFacade()) {
				...
				if (servletContext != null) {
					...
					throw new IllegalStateException(sm.getString(
							"coyoteRequest.dispatchNoServletContext", requestURI));
				}
			}
			resume();
		}
		...
		public void setRequestAndResponse(ServletRequest servletRequest,
		...
		public void addListener(AsyncListener listener, ServletRequest servletRequest,
				ServletResponse servletResponse) {
			asyncListeners.put(listener, new AsyncListenerRegistration(listener, servletRequest,
					servletResponse));
		}
		...
		public <T extends AsyncListener> T createListener(Class<T> clazz) throws ServletException {
			...
			try {
				...
				throw new ServletException(sm.getString("coyoteRequest.createListener",
						clazz.getName()), e);
			}
	...
	public class AsyncListenerRegistration {
		...
		protected AsyncListenerRegistration(AsyncListener listener, ServletRequest request,
