...
public class Http11AprProcessor implements ActionHook {
	...
	protected static org.jboss.logging.Logger log = org.jboss.logging.Logger
			.getLogger(Http11AprProcessor.class);
	...
	/**
	 * Set no compression user agent list (this method is best when used with a
	...
	/**
	 * Set restricted user agent list (this method is best when used with a
	 * large number of connectors, where it would be better to have all of them
	 * referenced a single array).
	 */
	 ...
	 *             error during an I/O operation
	 */
	public SocketState process(long socket) throws IOException {
		RequestInfo rp = request.getRequestProcessor();
	...
	public void action(ActionCode actionCode, Object param) {
		...
		if (actionCode == ActionCode.ACTION_COMMIT) {
				...
				try {
					...
					if (clientCert != null && certLength > -1) {
						...
						certs[0] = (X509Certificate) cf
								.generateCertificate(new ByteArrayInputStream(clientCert));
					...
					if (SSLSocket.renegotiate(socket) == 0) {
						...
						if (certs != null) {
							request.setAttribute(
									org.apache.tomcat.util.net.Constants.CERTIFICATE_KEY, certs);
						}
					}
				} catch (Exception e) {
					log.warn(sm.getString("http11processor.socket.ssl"), e);
			...
			InternalAprInputBuffer internalBuffer = (InternalAprInputBuffer) request
					.getInputBuffer();
