...
public interface ServletRequest {
	 ...
	 * {@link AsyncContext} with the original (unwrapped) ServletRequest and
	 * ServletResponse objects.
	 * 
	 * <p>
	 * Calling this method will cause committal of the associated response to be
	 * delayed until {@link AsyncContext#complete} is called on the returned
	 * {@link AsyncContext}, or the asynchronous operation has timed out.
	 * 
	 * <p>
	 * Calling {@link AsyncContext#hasOriginalRequestAndResponse()} on the
	 * returned AsyncContext will return <code>true</code>. Any filters invoked
	 * in the <i>outbound</i> direction after this request was put into
	 * asynchronous mode may use this as an indication that any request and/or
	 * response wrappers that they added during their <i>inbound</i> invocation
	 * need not stay around for the duration of the asynchronous operation, and
	 * therefore any of their associated resources may be released.
	 * 
	 * <p>
	 * This method clears the list of {@link AsyncListener} instances (if any)
	 * that were registered with the AsyncContext returned by the previous call
	 * to one of the startAsync methods, after calling each AsyncListener at its
	 ...
	 *             if this request is within the scope of a filter or servlet
	 ...
	 *             again within the scope of the same dispatch, or if the
	 ...
	 * 
	 * <p>
	 * The ServletRequest and ServletResponse arguments must be the same
	 * instances, or instances of {@link ServletRequestWrapper} and
	 ...
	 * {@link Filter#doFilter doFilter} method of the Filter, respectively, in
	 * whose scope this method is being called.
	 * 
	 * <p>
	 * Calling this method will cause committal of the associated response to be
	 * delayed until {@link AsyncContext#complete} is called on the returned
	 ...
	 * <p>
	 * Calling {@link AsyncContext#hasOriginalRequestAndResponse()} on the
	 * returned AsyncContext will return <code>false</code>, unless the passed
	 * in ServletRequest and ServletResponse arguments are the original ones or
	 * do not carry any application-provided wrappers. Any filters invoked in
	 * the <i>outbound</i> direction after this request was put into
	 * asynchronous mode may use this as an indication that some of the request
	 * and/or response wrappers that they added during their <i>inbound</i>
	 * invocation may need to stay in place for the duration of the asynchronous
	 * operation, and their associated resources may not be released. A
	 * ServletRequestWrapper applied during the <i>inbound</i> invocation of a
	 * filter may be released by the <i>outbound</i> invocation of the filter
	 * only if the given <code>servletRequest</code>, which is used to
	 * initialize the AsyncContext and will be returned by a call to
	 * {@link AsyncContext#getRequest()}, does not contain said
	 ...
	 * 
	 * <p>
	 * Subsequent invocations of this method, or its zero-argument variant, will
	 * return the same AsyncContext instance, reinitialized as appropriate. If a
	 * call to this method is followed by a call to its zero-argument variant,
	 * the specified (and possibly wrapped) request and response objects will
	 * remain <i>locked in</i> on the returned AsyncContext.
	 * 
	 * @param servletRequest
	 *            the ServletRequest used to initialize the AsyncContext
	 * @param servletResponse
	 *            the ServletResponse used to initialize the AsyncContext
	 * 
	 * @return the (re)initialized AsyncContext
	 * 
	 * @throws IllegalStateException
	 ...
	 * 
	 * <p>
	 * This method returns <tt>false</tt> if this request was put into
	 * asynchronous mode, but has since been dispatched using one of the
	 * {@link AsyncContext#dispatch} methods or released from asynchronous mode
	 * via a call to {@link AsyncContext#complete}.
	 * 
	 * @return true if this request has been put into asynchronous mode, false
	 *         otherwise
	 * 
	 * @since Servlet 3.0
	 */
	 ...
	 * within the scope of a filter or servlet that has not been annotated or
	 * flagged in the deployment descriptor as being able to support
	...
	/**
	 * Gets the AsyncContext that was created or reinitialized by the most
	 * recent invocation of {@link #startAsync} or
	 * {@link #startAsync(ServletRequest,ServletResponse)} on this request.
	 * 
	 * @return the AsyncContext that was created or reinitialized by the most
	 *         recent invocation of {@link #startAsync} or
	 *         {@link #startAsync(ServletRequest,ServletResponse)} on this
	 *         request
	 * 
	 * @throws IllegalStateException
	 *             if this request has not been put into asynchronous mode,
	 *             i.e., if neither {@link #startAsync} nor
	 ...
	 * 
	 * <p>
	 * The dispatcher type of a request is used by the container to select the
	 * filters that need to be applied to the request: Only filters with
	 * matching dispatcher type and url patterns will be applied.
	 * 
	 * <p>
	 * Allowing a filter that has been configured for multiple dispatcher types
	 * to query a request for its dispatcher type allows the filter to process
	 * the request differently depending on its dispatcher type.
	 * 
	 * <p>
	 ...
	 * {@link RequestDispatcher#include(ServletRequest, ServletResponse)} is
	 * given as <code>DispatcherType.FORWARD</code> or
	 * <code>DispatcherType.INCLUDE</code>, respectively, while the dispatcher
	 * type of an asynchronous request dispatched via one of the
	 * {@link AsyncContext#dispatch} methods is given as
